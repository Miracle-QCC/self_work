...
import pygame
import os
from pygame.locals import QUIT, KEYDOWN, K_ESCAPE, MOUSEBUTTONDOWN, MOUSEMOTION, MOUSEBUTTONUP, K_s # 导入事件
# 确保位于pygame初始化之前设置了窗口位置
os.environ['SDL_VIDEO_WINDOW_POS'] = "0,0"  # 这将窗口位置设置为屏幕左上角
class Brush:
    """
    画笔类
    """
...
class Menu:
    ...
    def __init__(self, screen):
        ...
        # 计算每个色块在画板中的坐标值，便于绘制
        self.colors_rect = []
        for (i, rgb) in enumerate(self.colors[:8]):  # 方块颜色表
            rect = pygame.Rect(254 + i * 32, 10,32, 32)
            self.colors_rect.append(rect)
        for (i, rgb) in enumerate(self.colors[8:]):  # 方块颜色表
            rect = pygame.Rect(254 + i * 32, 10 + 1* 32,32, 32)
            self.colors_rect.append(rect)
        self.pens = [  # 画笔图片
        ...
        for (i, img) in enumerate(self.erasers):  # 橡皮列表
            rect = pygame.Rect(10+(i + 1) * 64, 10 , 64, 64)
            self.erasers_rect.append(rect)

        self.pens_rect = []
        for (i, img) in enumerate(self.pens):  # 画笔列表
            rect = pygame.Rect(10, 10 + i * 32, 64, 64)
        ...
        # for (i, img) in enumerate(self.sizes):  # 橡皮列表
        rect1 = pygame.Rect(10 + 2 * 64, 10 , 64, 64)
        rect2 = pygame.Rect(10 + 2 * 64, 10 + 32, 64, 64)
        self.sizes_rect.append(rect1)
        self.sizes_rect.append(rect2)
    ...
    def draw(self):  # 绘制菜单栏
        ...
        # 绘制用于实时展示画笔的小窗口
        self.screen.fill((255, 255, 255), (10 + 64*2 + 35, 10, 64, 64))
        pygame.draw.rect(self.screen, (0, 0, 0), (10 + 64*2 + 35, 10, 64, 64), 1)
...
class Paint:
    ...
    def __init__(self):
        ...
        pygame.display.set_caption("陶依铭的超级画板") # 设置窗口标题
            ...
            for event in pygame.event.get(): # 遍历所有事件
                if event.type == QUIT:  # 退出事件
                    ...
                    if event.key == K_ESCAPE:  # ESC按键事件
                        self.clear_screen() 
                    elif event.key == K_s: # S保存
                        pygame.image.save(self.screen, "drawn_image.png")
                        print("保存成功")
