...
def sumtree_loss(q,q_targ,entroy):
    ...
    return torch.sqrt(torch.abs(q-q_targ) + 0.01*entroy.unsqueeze(-1))
...
class Dueling_Agent:
    ...
    def update(self, curious_net=None):
        ...
        with torch.no_grad():
                q_targ = reward + (((1-done) * self.gamma) * torch.max(self.q(next_state).detach(), dim=1)[0]).reshape(-1, 1)
