...
class PPO:
    def __init__(self, args, save_path):
        ...
        print("*********************88")
        print("self.recurrent {} ".format(self.recurrent))
        self.total_steps = 0
    ...
    def sample_parallel(self, env_fn, policy, critic, min_steps, max_traj_len, deterministic=False, anneal=1.0, term_thresh=0):
        ...
        while total_steps < min_steps:
            ...
            # print(" ├ len(result[-1]):      {}".format(len(result[-1])))
    ...
    def train(self,
        ...
        for itr in range(n_itr):
            ...
            if logger is not None:
                ...
                mean_losses1.append(mean_losses)
                data = {
                    'avg_eval_reward': avg_eval_rewards,
                    'avg_batch_reward': avg_batch_rewards,
                    'avg_ep_len': avg_ep_lens,
                    'mean_losses1': mean_losses1
                }
                # try:
                # 将字典转换为DataFrame
                ...
                # 指定保存的文件路径
                    # file_path = '/home/wzx/Lec11_Passive_walker/straight_step01.xlsx'
                    # df.to_excel(file_path, index=False)
                    # print("Data saved to Excel successfully.")
                # except Exception as e:
                    # print(f"Error while saving data to Excel: {e}")
                #####################
