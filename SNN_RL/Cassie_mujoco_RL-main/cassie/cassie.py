...
class CassieEnv:
    ...
    def step(self, action, return_omniscient_state=False, f_term=0):
            ...
            if False:
                ...
                if qpos[0] > 1.75:
                    ...
                    if intersection_point is not None:
                        ...
                        self.neutral_pelvis_orient = np.array([quaternion[0], quaternion[1], quaternion[2], quaternion[3]])
                        self.pelvis_orient_cost += (1 - np.inner(self.neutral_pelvis_orient, self.sim.xquat("pelvis")) ** 2)
                    self.pelvis_distance_cost = distance_to_circle(a, b, r, qpos[0], qpos[1])
                # if self.l_foot_pos[0] > 1.75:
                #     intersection_point = find_intersection_points(a, b, r, self.l_foot_pos[0], self.l_foot_pos[1])
                #     if intersection_point is not None:
                #         angle = central_angle(a, b, 1.75, 0, intersection_point[0], intersection_point[1])
                #         # 欧拉角表示 (roll, pitch, yaw) in 弧度
                #         euler_angles = (0, 0, angle)
                ...
                #         rotation = Rotation.from_euler('xyz', euler_angles)
                #         quaternion = rotation.as_quat()
                #         self.neutral_foot_l_orient = np.array([quaternion[0], quaternion[1], quaternion[2], quaternion[3]])
                #         self.l_foot_orient_cost += (1 - np.inner(self.neutral_foot_l_orient, self.sim.xquat("left-foot")) ** 2)
                # if self.r_foot_pos[0] > 1.75:
                #     intersection_point = find_intersection_points(a, b, r, self.r_foot_pos[0], self.r_foot_pos[1])
                #     if intersection_point is not None:
                #         angle = central_angle(a, b, 1.75, 0, intersection_point[0], intersection_point[1])
                #         # 欧拉角表示 (roll, pitch, yaw) in 弧度
                #         euler_angles = (0, 0, angle)
                ...
                #         rotation = Rotation.from_euler('xyz', euler_angles)
                #         quaternion = rotation.as_quat()
                #         self.neutral_foot_r_orient = np.array([quaternion[0], quaternion[1], quaternion[2], quaternion[3]])
                #         self.r_foot_orient_cost += (1 - np.inner(self.neutral_foot_r_orient, self.sim.xquat("right-foot")) ** 2)
        ...
        # if height < 0.4 or height > 5:#....................................
        height2 = (self.cassie_state.pelvis.position[2] - self.cassie_state.terrain.height)
        if height2 < 0.4 or height2 > 5:
            done = True
        else:
            done = False
    ...
    def get_full_state(self):
        ...
        if self.input_profile == "min":
            ...
            # print("**************************************\n")
            # print("pelvis.position: {:.2f} m".format(self.cassie_state.pelvis.position[2]))
            # print("terrain.height: {:.9f} m".format(self.cassie_state.terrain.height))
            # print("pelvis height: {:.2f} m".format(self.cassie_state.pelvis.position[2] - self.cassie_state.terrain.height))
            # print("**************************************\n")
        else:
            raise NotImplementedError
