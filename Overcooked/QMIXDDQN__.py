    ...
    def update(self, curious_net=None):
        ...
        with torch.no_grad():
            ...
            if curious_net:
                ...
                # q_targ1 = reward1 + (
                #             ((1 - done) * self.gamma) * (torch.max(self.q_targ1(next_state1).detach(), dim=1)[0])).reshape(-1, 1)
                # q_targ2 = reward2 + (
                #         ((1 - done) * self.gamma) * (
                #             torch.max(self.q_targ2(next_state2).detach(), dim=1)[0])).reshape(-1, 1)
                q_targ1, q_targ2 = torch.max(self.q_targ1(next_state1).detach(), dim=1)[0], \
                                   torch.max(self.q_targ2(next_state2.detach()), dim=1)[0]
                q_total_target = reward1 + reward2 + (1 - done) * self.gamma * self.qmix(
                    torch.cat([q_targ1, q_targ2], dim=-1), next_state).view(-1, 1) + curious_value * 0.1
            else:
                ...
                q_targ1, q_targ2 = torch.max(self.q_targ1(next_state1).detach(), dim=1)[0], \
                                   torch.max(self.q_targ2(next_state2.detach()), dim=1)[0]
                q_total_target = reward1 + reward2 + (1 - done) * self.gamma * self.qmix(
                    torch.cat([q_targ1, q_targ2], dim=-1), next_state).view(-1, 1)
