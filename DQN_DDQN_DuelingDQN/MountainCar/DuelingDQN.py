...
SEED = 0
MODEL_PATH = ''
SAVE_PATH_PREFIX = 'log/duelingdqn/'
TEST = False
MAX_STEPS = 300
...
class DQN():
    ...
    def learn(self):
        ...
        with torch.no_grad():
            q_targ = rewards + (((1-dones) * GAMMA) * torch.max(self.eval_net(next_states).detach(), dim=1)[0]).reshape(-1, 1)
...
def main():
    ...
    for i in range(EPISODES):
        ...
        while True:
            ...
            r = 100 * ((np.sin(3 * next_state[0]) * 0.0025 + 0.5 * next_state[1] * next_state[1]) - (
                    np.sin(3 * state[0]) * 0.0025 + 0.5 * state[1] * state[1]))

            # next_state, reward, done, info = env.step(action)
            if isinstance(next_state, tuple):
                next_state = next_state[0]
            dqn.store_transition(Data(state, action, r, next_state, done))
            ep_reward += reward
            if TEST:
                env.render()
    ...
    plt.title('DuelingDQN MountainCar Reward per Episode')
    # plt.show()
    plt.savefig('DuelingDQN_MountainCar.png')
